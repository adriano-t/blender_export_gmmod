bl_info = {
        "name": "Export for GameMaker (.gmmod or .gml)",
        "author": "Tiz",
        "version": (1, 0, 0),
        "blender": (2, 80, 0),
        "location": "File > Export",
        "description": "Export 3D models for use GameMaker: Studio, as gmmod or script",
        "warning": "",
        "wiki_url": "",
        "tracker_url": "",
...
Additional credits:
    originally created by martincrownover for blender 2.7.x
...
def prepMesh(object, apply_modifiers):
        ...
        for i in bpy.context.scene.objects: i.select_set(False)
        ...
        bpy.context.scene.collection.objects.link(object_copy)
...
def do_export(context, props, filepath):
        ...
        if props.output_format == 'SCRIPT':
                writeString(file, 'NOT IMPLEMENTED!\n')
        # #script output
        ...
        #                         uv_layer = mesh.uv_layers.active #or 0
                        ...
                        if len(mesh.uv_layers) > 0:
                                ...
                                for tri in mesh.loop_triangles:
                                        ...
                                        for index in tri.loops:
                                                ...
                                                if props.flip_uvs:
                                                        model_output_string += '%f     %f     ' % (faceUV[0], 1-faceUV[1])
                                                else:
                                                        model_output_string += '%f     %f     ' % (faceUV[0], faceUV[1])
        ...
        bpy.context.scene.collection.objects.unlink(object_copy)
...
class EXPORT_OT_gm3d(bpy.types.Operator, ExportHelper):
        """Exports the active object as a GameMaker: Studio 3D model script"""
        bl_idname = "export.gmmod"
        bl_label = "Export for GameMaker: Studio"
        filename_ext = ".gmmod"

        filter_glob = StringProperty(default="*.gmmod", options={'HIDDEN'})
...
def menu_func_export(self, context):
        self.layout.operator(EXPORT_OT_gm3d.bl_idname, text="GameMaker 3D (.gmmod)")

classes = (EXPORT_OT_gm3d,)
